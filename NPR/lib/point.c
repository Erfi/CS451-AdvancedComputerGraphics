...
Vector vector_add(Vector *a, Vector *b){
    if(NULL != a && NULL != b){
        Vector sum;
        vector_set(&sum,a->val[0]+b->val[0],a->val[1] + b->val[1], a->val[2] + b->val[2]);
        return sum;
    }
...
Vector vector_subtract(Vector *a, Vector *b){
    if(NULL != a && NULL != b){
        Vector result;
        vector_set(&result, a->val[0]-b->val[0], a->val[1]-b->val[1], a->val[2]-b->val[2]);
        return result;
    }
}
...
Vector vector_scale(Vector* vector, double t){
    if(vector != NULL){
        Vector result;
        vector_set(&result, vector->val[0] * t, vector->val[1] * t, vector->val[2] * t);
        return result;
    }
}
